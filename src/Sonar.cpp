...
base::samples::Sonar Sonar::simulateSonar(const std::vector<float>& bins, float range) {
    ...
    //sonar.setRegularBeamBearings(-beam_width/2, beam_width/beam_count);
    std::vector<base::Angle> &bearings = sonar.bearings;
    bearings.resize(beam_count);

    double beamPerAng = beam_width.rad/beam_count, startAng = -beam_width.rad/2;
    for(unsigned i=0; i<beam_count; i++){
        sonar.bearings[i] = base::Angle::fromRad(startAng + i*beamPerAng);
    }
...
std::vector<int> Sonar::getBinsDepth(cv::Mat& cv_image, std::vector<float>& bins) {
    // calculate depth histogram
    std::vector<int> bins_depth(cv_image.cols * cv_image.rows, 0);
    float* ptr = reinterpret_cast<float*>(cv_image.data);
    for (int i = 0; i < cv_image.cols; i++) {
        for (int j = 0; j < cv_image.rows; j++) {
            std::cout << cv_image.at<double>(i, j, 0) << '\n';
        }
        std::cout << '\n';
    }

    std::cout << cv_image.cols;
    std::cout << '\n';
    std::cout << cv_image.rows;
